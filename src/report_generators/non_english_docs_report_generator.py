...
class NonEnglishDocsReportGenerator(BaseReportGenerator):
    @property
    def headers(self):
        return self.base_headers() + ["text", "text_languages", "detected_as_english"]
    ...
    def process_page(self, content_item, html):
        if pd.isna(content_item['text']):
            return self.base_columns(content_item, html) + [content_item['text'], [], content_item.get('detected_as_english',
                                                                                          default=False)]
